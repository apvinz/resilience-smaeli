...
import { Plus, Trash2, RefreshCw, Camera, Image as ImageIcon } from "lucide-react";
import { motion, AnimatePresence } from "motion/react";

import { useAuth } from "../../context/AuthContext";

interface ClassDocumentationSectionProps {
  level: string;
  subclass: string;
}

export const ClassDocumentationSection: React.FC<ClassDocumentationSectionProps> = ({ level, subclass }) => {
  ...
  const photos = currentClassData?.photos || [];
  ...
  const handleAddPhoto = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        ...
        updateClassData(subclassDisplay, { photos: updatedPhotos });
        showToast("Foto dokumentasi kelas ditambahkan! Klik SIMPAN di kanan atas jika sudah selesai.");
        if (fileInputRef.current) fileInputRef.current.value = "";
      } catch (err) {
        showToast("Gagal mengunggah foto dokumentasi kelas", "error");
      }
    }
  };
    ...
    <section id="documentation" className="py-20 px-6 bg-[#0E1015] border-b border-white/5 select-none">
      <div className="max-w-[1200px] mx-auto space-y-12">
        ...
        <div className="text-center relative max-w-sm mx-auto">
          ...
          <span className="text-[10px] font-black tracking-[0.2em] text-[#FF7A00] uppercase block mt-1">
            Momen & Kebersamaan {subclassDisplay}
          </span>
        ...
        {photos.length === 0 ? (
          <div className="text-center py-20 bg-[#14161E] rounded-2xl shadow-card border border-white/10 space-y-3">
            ...
            <p className="text-gray-300 font-semibold">Belum ada foto dokumentasi kelas yang ditambahkan</p>
            {editMode && (
                    ...
                    <div className={`absolute inset-0 bg-black/60 flex flex-col items-center justify-center gap-3 transition-opacity duration-200 ${
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="p-2 bg-[#FF7A00] text-white rounded-full hover:bg-[#E06600] transition-all transform hover:scale-110 shadow cursor-pointer"
                          title="Ganti Foto"
                        >
                          <RefreshCw size={14} />
                        </button>
                        ...
                        <button
                          ...
                          className="p-2 bg-[#EF4444] text-white rounded-full hover:bg-red-600 transition-all transform hover:scale-110 shadow cursor-pointer"
                          title="Hapus Foto"
                        >
                          <Trash2 size={14} />
                        </button>
        ...
        {editMode && (
          <div className="flex justify-center pt-4">
            <button
              ...
              className="flex items-center gap-2 px-6 py-3.5 bg-gradient-to-r from-[#FF7A00] to-[#FF8C00] text-white text-xs font-black uppercase tracking-wider rounded-full shadow-lg hover:shadow-xl transition-all cursor-pointer hover:scale-102"
            >
              <Plus size={16} />
              <span>Tambah Foto Dokumentasi Kelas</span>
            </button>
            <input
              type="file"
              ref={fileInputRef}
